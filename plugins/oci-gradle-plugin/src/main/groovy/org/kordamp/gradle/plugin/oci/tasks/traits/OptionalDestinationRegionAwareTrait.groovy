...
trait OptionalDestinationRegionAwareTrait implements PathAware, ProjectAware {
    private final StringState state = SimpleStringState.of(project, this, 'oci.destination.region')
    ...
    @Input
    @Optional
    Provider<String> getResolvedDestinationRegion() {
        state.provider
    }

    @Option(option = 'destination-region', description = 'The name of the destination Region (OPTIONAL).')
    void setDestinationRegion(String destinationRegion) {
        getDestinationRegion().set(destinationRegion)
    }
}
