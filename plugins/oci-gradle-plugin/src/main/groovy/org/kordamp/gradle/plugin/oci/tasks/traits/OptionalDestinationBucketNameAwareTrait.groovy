...
trait OptionalDestinationBucketNameAwareTrait implements PathAware, ProjectAware {
    private final StringState state = SimpleStringState.of(project, this, 'oci.destination.bucket.name')
    ...
    @Input
    @Optional
    Provider<String> getResolvedDestinationBucketName() {
        state.provider
    }

    @Option(option = 'destination-bucket-name', description = 'The name of the destination Bucket (OPTIONAL).')
    void setDestinationBucketName(String destinationBucketName) {
        getDestinationBucketName().set(destinationBucketName)
    }
}
