 ...
 */
@CompileStatic
trait GatewayIdAwareTrait implements PathAware, ProjectAware {
    ...
    @Option(option = 'gateway-id', description = 'The id of the gateway (REQUIRED).')
    void setGatewayId(String gatewayId) {
        getGatewayId().set(gatewayId)
    }

    void validateGatewayId() {
        if (isBlank(getResolvedGatewayId().orNull)) {
            throw new IllegalStateException("Missing value for 'gatewayId' in $path")
        }
        if (!isValid(getResolvedGatewayId().get())) {
