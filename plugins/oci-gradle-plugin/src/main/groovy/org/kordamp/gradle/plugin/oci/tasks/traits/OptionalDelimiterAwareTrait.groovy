...
trait OptionalDelimiterAwareTrait implements PathAware, ProjectAware {
    private final StringState state = SimpleStringState.of(project, this, 'oci.delimiter')
    ...
    @Input
    @Optional
    Provider<String> getResolvedDelimiter() {
        state.provider
    }

    @Option(option = 'delimiter', description = 'The delimiter parameter (OPTIONAL).')
    void setDelimiter(String delimiter) {
        getDelimiter().set(delimiter)
    }
}
